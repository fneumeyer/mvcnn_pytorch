...
class SpaPGNet(Model):
    ...
    def __init__(self, name, latent_space = 2048):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose3d(latent_space, 512, 2),
            #torch.nn.BatchNorm3d(1024),
            nn.ReLU(),
            nn.ConvTranspose3d(512, 512, 2),
            #torch.nn.BatchNorm3d(512),
            nn.ReLU(),
            nn.ConvTranspose3d(512, 512, 2),
            #torch.nn.BatchNorm3d(256),
            nn.ReLU(),
            nn.ConvTranspose3d(512, 256, 3),
            #torch.nn.BatchNorm3d(128),
            nn.ReLU(),
            nn.ConvTranspose3d(256, 256, 3),
            #torch.nn.BatchNorm3d(64),
            nn.ReLU(),            
            nn.ConvTranspose3d(256, 64, 5, stride = 3, padding = 2),
            #torch.nn.BatchNorm3d(32),
            nn.ReLU(),
            nn.ConvTranspose3d(64, 1, 5, stride = 3, padding = 2),
            #nn.ReLU(),
        )
